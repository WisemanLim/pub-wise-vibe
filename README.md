...
| 파일 | 설명 |
|------|------|
| `install.sh` | curl 원라인 설치 진입점. 로컬 `~/.wise_vibe/share/` 에 `.env.example`, `setup_vibe.sh`, `review_source.sh` 다운로드. `setup_vibe` 는 로컬 **setup_vibe.sh** 실행 |
| `setup_vibe.sh` | 바이브 코딩 서비스 선택(1~5)·API 키 입력·Hi 테스트·.env 저장. 설치 시 share로 복사되어 터미널에서 정상 입력 동작 |
...
- **로컬에 다운로드되는 파일**: `~/.wise_vibe/share/` 에 **.env.example**, **setup_vibe.sh**, **review_source.sh** 가 저장됩니다.
- **setup_vibe**: `setup_vibe` 명령은 로컬 `~/.wise_vibe/share/setup_vibe.sh` 를 실행합니다. (파이프 없이 실행되어 터미널에서 메뉴·입력이 정상 동작합니다.)
...
- **동작 방식**: 로컬 `~/.wise_vibe/share/setup_vibe.sh` 를 실행합니다.
