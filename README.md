...
| 파일 | 설명 |
|------|------|
| `install.sh` | **단일 진입점.** curl 원라인 설치 + 바이브 환경 구성. `~/.wise_vibe/share/` 에 `.env.example`, `review_source.sh` 다운로드. 설치 직후 서비스 선택(1~5)·API 키·.env 저장 진행. `setup_vibe` 명령은 원격 `install.sh --setup` 실행 |
...
- **로컬에 다운로드되는 파일**: `~/.wise_vibe/share/` 에 **.env.example**, **review_source.sh** 가 저장됩니다.
- **setup_vibe**: `setup_vibe` 명령은 원격 `install.sh --setup` 을 실행해 서비스 선택 메뉴를 다시 띄웁니다. (터미널에서 정상 입력 가능.)
...
- **실행 명령**: `setup_vibe` (PATH에 등록). 원격 `install.sh --setup` 을 호출해 1~5번 서비스 선택, API 키·추가 정보 입력, 'Hi' 테스트, `.env` 저장을 진행합니다.
...
- **동작 방식**: 원격 `install.sh --setup` 을 호출합니다. (설치 시와 동일한 설정 플로우.)
