...
| 파일 | 설명 |
|------|------|
| `install.sh` | **단일 진입점.** curl 원라인 설치 + 바이브 환경 구성. `~/.wise_vibe/share/` 에 `.env.example`, `review_source.sh`, `install.sh` 다운로드. 설치 직후 서비스 선택(1~5)·API 키·.env 저장. `setup_vibe` 는 로컬 `install.sh --setup` 실행 |
...
| `review_all.sh` | GitHub 사용자 스타 저장소 목록 조회 후 각 repo에 대해 `review_source` 실행 (자동 G 선택). `./review_all.sh [username]` |
| `README.md` | 본 문서 |
| `LICENSE` | MIT |
...
- **로컬에 다운로드되는 파일**: `~/.wise_vibe/share/` 에 **.env.example**, **review_source.sh**, **install.sh** 가 저장됩니다.
...
- **역할**: 해당 사용자의 **스타 저장소** 목록을 GitHub API로 가져온 뒤, 각 저장소마다 `review_source <url>` 를 실행합니다. AI 선택 시 **G**(Gemini)를 자동 입력합니다.
- **요구**: `curl`, `jq`(권장, 없으면 grep/sed로 파싱 시도). `GITHUB_TOKEN`(선택) 설정 시 API 제한 완화.
