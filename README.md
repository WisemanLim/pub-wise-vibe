...
| 파일 | 설명 |
|------|------|
| `install.sh` | curl 원라인 설치 진입점. 설치 모드 시 로컬에 `.env.example`, `review_source.sh`만 다운로드. `setup_vibe` 동작은 **원격 install.sh**를 통해서만 수행됨 |
...
| `review_source.sh` | GitHub 저장소 클론 → AI 리뷰(Gemini/Claude/Both) → REVIEW.md 및 review_addon 생성 |
| `README.md` | 본 문서 |
| `LICENSE` | MIT |
...
- **로컬에 다운로드되는 파일**: `~/.wise_vibe/share/` 에 **.env.example**, **review_source.sh** 만 저장됩니다.
- **setup_vibe**: 로컬에 별도 스크립트를 두지 않습니다. `setup_vibe` 명령 실행 시마다 위 원격 `install.sh`를 호출해, 그 안의 setup_vibe 로직만 수행합니다.
...
**참고**: `setup_vibe` 실행 후 메뉴가 안 뜨고 바로 종료되면, **설치를 한 번 더** 실행한 뒤 `setup_vibe`를 다시 실행하세요.  
`curl -fsSL .../install.sh | bash` 를 다시 실행하면 `~/.wise_vibe/bin/setup_vibe` 와 원격 install.sh 내용이 최신으로 갱신됩니다.
...
`setup_vibe` 실행 시 원격 install.sh가 내려와 1~5번 서비스 선택, API 키·추가 정보 입력, 'Hi' 테스트, `.env` 저장까지 진행합니다.
...
- **리뷰**: `review_source` 는 로컬에 설치된 `review_source.sh` 를 사용하며, Gemini/Claude CLI가 있으면 AI 분석을 수행합니다.
...
- **동작 방식**: 매번 원격 `install.sh` 를 `setup_vibe` 인자로 호출해 수행합니다.

### review_source

- **사용법**: `review_source <github_url> [url2 ...]`
- **역할**:
  1. GitHub 저장소 클론(여러 URL 시 저장소 이름으로 디렉터리 생성).
  2. AI 리뷰 서비스 선택: **G**emini / **C**laude / **B**oth(비교). 환경에 따라 설치 여부(✓/✗) 표시.
  3. clone한 저장소 경로에서 **REVIEW.md** 생성(구조 다이어그램, 구성 요소, AI 상세 분석). Both 선택 시 **REVIEW_gemini.md**, **REVIEW_claude.md** 와 **REVIEW.md** 비교 요약.
  4. 모든 과정 후 상위 디렉터리로 `cd ..` 복귀.
  5. **review_addon** 생성: 배포·보안·CI 등 권고 addon.
...
```
<repo_name>/
├── REVIEW.md              # 구조 다이어그램 + AI 분석 (단일 선택 시) 또는 G/C 비교 요약 (Both 시)
...
- **REVIEW.md 내용**: 트리 구조, FE/BE/DB·설치 파일 수, AI(Gemini/Claude)별 구조 요약·디렉터리별 기능·호출 관계·BE API curl 예제·총론(개요·부족점·HTTPS·Docker·보안 권고).
